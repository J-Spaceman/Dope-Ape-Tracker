import { useState, useEffect, useRef, useMemo } from "react";
import * as d3 from "d3";

const CONTRACT = "0x4b570b636e4F744199ec82F52d69B08b394ab850";
const LOGO_SRC = "data:image/png;base64,";

const MARKETPLACES = {
  "0x7be8076f4ea4a4ad08075c2508e481d6c946d12b": "OpenSea v1",
  "0x00000000006c3852cbef3e08e8df289169ede581": "Seaport",
  "0x000000000000ad05ccc4f10045630fb830b95127": "Blur",
  "0x59728544b08ab483533076417fbbb2fd0b17ce3a": "LooksRare",
  "0x74312363e45dcaba76c59ec49a7aa8a65a67eed": "X2Y2",
  "0x0000000000000000000000000000000000000000": "Mint",
};
const isMP   = (a) => !!MARKETPLACES[a?.toLowerCase()];
const short  = (a) => a ? `${a.slice(0,6)}…${a.slice(-4)}` : "";
const rndHex = (n) => [...Array(n)].map(()=>Math.floor(Math.random()*16).toString(16)).join("");

const DEMO = (() => {
  const now = Math.floor(Date.now()/1000);
  const ws  = Array.from({length:42},()=>"0x"+rndHex(40));
  const txs = [];
  for(let i=0;i<220;i++){
    let fi=Math.floor(Math.random()*ws.length),ti=Math.floor(Math.random()*ws.length);
    while(ti===fi) ti=Math.floor(Math.random()*ws.length);
    txs.push({from:ws[fi],to:ws[ti],tokenID:String(Math.floor(Math.random()*4999)+1),
      timeStamp:String(now-Math.floor(Math.random()*180*86400)),
      hash:"0x"+rndHex(64),marketplace:Math.random()>0.42});
  }
  const w1=ws[0];
  for(let i=0;i<7;i++) txs.push({from:w1,to:ws[Math.floor(Math.random()*(ws.length-1))+1],
    tokenID:String(1000+i),timeStamp:String(now-Math.floor(Math.random()*28800)),
    hash:"0x"+rndHex(64),marketplace:true});
  const w2=ws[1];
  for(let i=0;i<5;i++) txs.push({from:ws[Math.floor(Math.random()*(ws.length-2))+2],to:w2,
    tokenID:String(2000+i),timeStamp:String(now-Math.floor(Math.random()*86400)),
    hash:"0x"+rndHex(64),marketplace:true});
  return txs;
})();

const nodeR = (d) => d.isMP ? 14 : Math.max(5,Math.min(14,4+(d.sent+d.received)*0.6));
const nodeColor = (d,alertSet) => {
  if(alertSet.has(d.id)) return "#F5C300";
  if(d.isMP)             return "#888";
  const a=d.sent+d.received;
  if(a>10) return "#FFF";
  if(a>4)  return "#CCC";
  return "#666";
};

const Tile = ({label,value,accent}) => (
  <div style={{flex:1,minWidth:0,background:"#111",borderRadius:12,border:"1px solid #222",
    padding:"13px 14px",borderTop:`2px solid ${accent}`,position:"relative",overflow:"hidden"}}>
    <div style={{position:"absolute",top:0,left:0,right:0,bottom:0,
      background:`radial-gradient(ellipse at top left,${accent}0A 0%,transparent 60%)`,pointerEvents:"none"}}/>
    <div style={{fontSize:8,color:"rgba(255,255,255,0.28)",letterSpacing:"0.1em",marginBottom:6}}>{label}</div>
    <div style={{fontSize:22,fontWeight:700,color:"#fff",fontFamily:"'DM Mono',monospace",letterSpacing:"-0.02em"}}>{value}</div>
  </div>
);

export default function DAC() {
  const [transfers,setTransfers] = useState(DEMO);
  const [loading,setLoading]     = useState(false);
  const [demoMode,setDemoMode]   = useState(true);
  const [filterMode,setFilterMode]=useState("all");
  const [selected,setSelected]   = useState(null);
  const [alerts,setAlerts]       = useState([]);
  const [aiAnalysis,setAiAnalysis]=useState(null);
  const [aiLoading,setAiLoading] = useState(false);
  const [apiKey,setApiKey]       = useState("AZ22N7MWSRDG4934C48KYR2QXEF57M4QW7");
  const [showInput,setShowInput] = useState(false);
  const [error,setError]         = useState(null);
  const [isMobile,setIsMobile]   = useState(false);
  const svgRef = useRef(null);

  // Responsive breakpoint
  useEffect(()=>{
    const check = () => setIsMobile(window.innerWidth < 768);
    check();
    window.addEventListener("resize", check);
    return () => window.removeEventListener("resize", check);
  },[]);

  const computeAlerts = (txs) => {
    const now=Date.now()/1000, cnt={};
    txs.forEach(tx=>{
      if(now-Number(tx.timeStamp)<=48*3600)
        [tx.from,tx.to].forEach(a=>{
          const lc=a.toLowerCase();
          if(!isMP(lc)&&lc!=="0x0000000000000000000000000000000000000000")
            cnt[lc]=(cnt[lc]||0)+1;
        });
    });
    return Object.entries(cnt).filter(([,n])=>n>2)
      .sort((a,b)=>b[1]-a[1]).slice(0,8).map(([addr,count])=>({addr,count}));
  };
  useEffect(()=>{ setAlerts(computeAlerts(DEMO)); },[]);

  useEffect(()=>{
    const autoFetch = async () => {
      try {
        const url=`https://api.etherscan.io/v2/api?chainid=1&module=account&action=tokennfttx&contractaddress=${CONTRACT}&page=1&offset=1000&sort=desc&apikey=AZ22N7MWSRDG4934C48KYR2QXEF57M4QW7`;
        const res=await fetch(url); const json=await res.json();
        if(json.status==="1"&&json.result?.length>0){
          const txs=json.result.map(tx=>({...tx,marketplace:isMP(tx.from)||isMP(tx.to)}));
          setTransfers(txs); setAlerts(computeAlerts(txs)); setDemoMode(false);
        }
      } catch{}
    };
    autoFetch();
  },[]);

  const fetchLive = async () => {
    if(!apiKey.trim()) return;
    setLoading(true); setError(null);
    try {
      const url=`https://api.etherscan.io/v2/api?chainid=1&module=account&action=tokennfttx&contractaddress=${CONTRACT}&page=1&offset=1000&sort=desc&apikey=${apiKey}`;
      const res=await fetch(url); const json=await res.json();
      if(json.status==="1"&&json.result?.length>0){
        const txs=json.result.map(tx=>({...tx,marketplace:isMP(tx.from)||isMP(tx.to)}));
        setTransfers(txs); setAlerts(computeAlerts(txs)); setDemoMode(false); setShowInput(false);
      } else { setError(json.message||"No data — check key"); }
    } catch(e){ setError(e.message); }
    setLoading(false);
  };

  const analyzeWallet = async (w) => {
    if(!w) return;
    setAiLoading(true); setAiAnalysis(null);
    try {
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{
          role:"user",content:`Analyze this NFT wallet from Dope Ape Club: Address:${w.id} Sent:${w.sent} Received:${w.received} Unique tokens:${w.tokens?.size||0} Is marketplace:${w.isMP} Whale alert:${alerts.some(a=>a.addr===w.id)}. In 2 sentences classify (whale/flipper/collector/dev/marketplace) and describe behavior. Be direct.`
        }]})
      });
      const data=await res.json();
      setAiAnalysis(data.content?.find(b=>b.type==="text")?.text||"Unable to analyze.");
    } catch { setAiAnalysis("AI analysis unavailable."); }
    setAiLoading(false);
  };

  const graph = useMemo(()=>{
    let txs=transfers;
    if(filterMode==="buysell")    txs=transfers.filter(t=>t.marketplace);
    if(filterMode==="walletonly") txs=transfers.filter(t=>!t.marketplace);
    const nm={},lm={};
    txs.forEach(tx=>{
      const f=tx.from.toLowerCase(),t=tx.to.toLowerCase();
      [f,t].forEach(a=>{if(!nm[a]) nm[a]={id:a,sent:0,received:0,tokens:new Set(),isMP:isMP(a),name:MARKETPLACES[a]};});
      nm[f].sent++; nm[f].tokens.add(tx.tokenID);
      nm[t].received++; nm[t].tokens.add(tx.tokenID);
      const k=`${f}→${t}`;
      if(!lm[k]) lm[k]={source:f,target:t,count:0};
      lm[k].count++;
    });
    const nodes=Object.values(nm).sort((a,b)=>(b.sent+b.received)-(a.sent+a.received)).slice(0,90);
    const ids=new Set(nodes.map(n=>n.id));
    return {nodes,links:Object.values(lm).filter(l=>ids.has(l.source)&&ids.has(l.target))};
  },[transfers,filterMode]);

  useEffect(()=>{
    if(!svgRef.current||graph.nodes.length===0) return;
    const el=svgRef.current,W=el.clientWidth||600,H=el.clientHeight||400;
    d3.select(el).selectAll("*").remove();
    const svg=d3.select(el);
    const defs=svg.append("defs");
    [["gy","0.96,0.76,0"],["gw","1,1,1"],["gg","0.5,0.5,0.5"]].forEach(([id,rgb])=>{
      const [r,g,b]=rgb.split(",");
      const f=defs.append("filter").attr("id",id);
      f.append("feGaussianBlur").attr("in","SourceGraphic").attr("stdDeviation",3.5).attr("result","bl");
      f.append("feColorMatrix").attr("in","bl").attr("type","matrix")
        .attr("values",`0 0 0 0 ${r}  0 0 0 0 ${g}  0 0 0 0 ${b}  0 0 0 14 -5`).attr("result","col");
      const m=f.append("feMerge"); m.append("feMergeNode").attr("in","col"); m.append("feMergeNode").attr("in","SourceGraphic");
    });
    const g=svg.append("g");
    svg.call(d3.zoom().scaleExtent([0.05,14]).on("zoom",e=>g.attr("transform",e.transform)));
    const alertSet=new Set(alerts.map(a=>a.addr));
    const simNodes=graph.nodes.map(n=>({...n}));
    const simLinks=graph.links.map(l=>({...l}));
    const sim=d3.forceSimulation(simNodes)
      .force("link",d3.forceLink(simLinks).id(d=>d.id).distance(d=>50+d.count*9).strength(0.38))
      .force("charge",d3.forceManyBody().strength(d=>d.isMP?-280:-65))
      .force("center",d3.forceCenter(W/2,H/2))
      .force("collide",d3.forceCollide(d=>nodeR(d)+6));
    const link=g.append("g").selectAll("line").data(simLinks).join("line")
      .attr("stroke",d=>d.count>2?"rgba(245,195,0,0.18)":"rgba(255,255,255,0.06)")
      .attr("stroke-width",d=>Math.min(d.count*0.6,2));
    const nodeG=g.append("g").selectAll("g").data(simNodes).join("g")
      .style("cursor","pointer")
      .call(d3.drag()
        .on("start",(e,d)=>{if(!e.active)sim.alphaTarget(0.3).restart();d.fx=d.x;d.fy=d.y;})
        .on("drag",(e,d)=>{d.fx=e.x;d.fy=e.y;})
        .on("end",(e,d)=>{if(!e.active)sim.alphaTarget(0);d.fx=null;d.fy=null;})
      )
      .on("click",(e,d)=>{e.stopPropagation();setSelected(graph.nodes.find(n=>n.id===d.id)||null);setAiAnalysis(null);});
    nodeG.filter(d=>alertSet.has(d.id)).append("circle")
      .attr("r",d=>nodeR(d)+8).attr("fill","none")
      .attr("stroke","rgba(245,195,0,0.2)").attr("stroke-width",1.5);
    nodeG.append("circle")
      .attr("r",d=>nodeR(d))
      .attr("fill",d=>nodeColor(d,alertSet))
      .attr("fill-opacity",0.9)
      .attr("filter",d=>alertSet.has(d.id)?"url(#gy)":d.isMP?"url(#gg)":"url(#gw)")
      .attr("stroke",d=>alertSet.has(d.id)?"rgba(245,195,0,0.6)":d.isMP?"rgba(150,150,150,0.35)":"rgba(255,255,255,0.25)")
      .attr("stroke-width",1);
    nodeG.filter(d=>d.isMP||(d.sent+d.received>8))
      .append("text")
      .text(d=>d.isMP?(d.name||short(d.id)):short(d.id))
      .attr("font-size","7px").attr("font-family","'DM Mono',monospace")
      .attr("fill","rgba(255,255,255,0.28)").attr("text-anchor","middle")
      .attr("dy",d=>-nodeR(d)-5).attr("pointer-events","none");
    sim.on("tick",()=>{
      link.attr("x1",d=>d.source.x).attr("y1",d=>d.source.y)
          .attr("x2",d=>d.target.x).attr("y2",d=>d.target.y);
      nodeG.attr("transform",d=>`translate(${d.x??0},${d.y??0})`);
    });
    svg.on("click",()=>{setSelected(null);setAiAnalysis(null);});
    return ()=>sim.stop();
  },[graph,alerts]);

  const stats=useMemo(()=>({
    txs:transfers.length,
    wallets:new Set([...transfers.map(t=>t.from),...transfers.map(t=>t.to)]).size,
    tokens:new Set(transfers.map(t=>t.tokenID)).size,
    mkt:transfers.filter(t=>t.marketplace).length,
  }),[transfers]);

  const card={background:"#0A0A0A",borderRadius:14,border:"1px solid #1C1C1C",overflow:"hidden"};
  const fBtn=(m,l)=>(
    <button key={m} onClick={()=>setFilterMode(m)} style={{
      padding: isMobile?"4px 9px":"5px 12px",
      borderRadius:7,cursor:"pointer",fontSize: isMobile?8:9,
      fontFamily:"'DM Mono',monospace",letterSpacing:"0.04em",transition:"all 0.15s",whiteSpace:"nowrap",
      background: filterMode===m?"rgba(245,195,0,0.1)":"transparent",
      border:`1px solid ${filterMode===m?"rgba(245,195,0,0.4)":"rgba(255,255,255,0.07)"}`,
      color: filterMode===m?"#F5C300":"rgba(255,255,255,0.28)",
    }}>{l}</button>
  );

  // Inspector panel — shared between layouts
  const InspectorPanel = () => (
    <div style={{...card,padding:"15px"}}>
      <div style={{fontSize:9,color:"rgba(255,255,255,0.22)",letterSpacing:"0.1em",
        marginBottom:12,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <span>WALLET INSPECTOR</span>
        {selected&&<span onClick={()=>{setSelected(null);setAiAnalysis(null);}}
          style={{cursor:"pointer",fontSize:11,color:"rgba(255,255,255,0.15)"}}>✕</span>}
      </div>
      {selected?(
        <div style={{animation:"fadein 0.2s ease"}}>
          <div style={{fontSize:9,color:"#F5C300",wordBreak:"break-all",
            padding:"7px 9px",background:"rgba(245,195,0,0.05)",
            border:"1px solid rgba(245,195,0,0.12)",borderRadius:8,marginBottom:11}}>
            {selected.isMP?`🏪 ${selected.name||short(selected.id)}`:selected.id}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:7,marginBottom:11}}>
            {[["SENT",selected.sent,"#F5C300"],["RECV",selected.received,"#fff"],["TOKENS",selected.tokens?.size||0,"#888"]].map(([k,v,c])=>(
              <div key={k} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",
                borderTop:`2px solid ${c}22`,padding:"9px 5px",borderRadius:8,textAlign:"center"}}>
                <div style={{fontSize:16,color:c,fontWeight:"bold"}}>{v}</div>
                <div style={{fontSize:8,color:"rgba(255,255,255,0.22)",marginTop:2}}>{k}</div>
              </div>
            ))}
          </div>
          {alerts.some(a=>a.addr===selected.id)&&(
            <div style={{marginBottom:9,padding:"7px 9px",borderRadius:8,
              background:"rgba(245,195,0,0.06)",border:"1px solid rgba(245,195,0,0.18)",
              fontSize:9,color:"#F5C300"}}>⚠ High movement in 48h window</div>
          )}
          <button onClick={()=>analyzeWallet(selected)} disabled={aiLoading} style={{
            width:"100%",padding:"8px",borderRadius:8,cursor:"pointer",
            background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.12)",
            color:"rgba(255,255,255,0.6)",fontSize:9,fontFamily:"inherit",letterSpacing:"0.06em",
            opacity:aiLoading?0.55:1,transition:"all 0.15s"}}>
            {aiLoading?"ANALYSING…":"⬡  AI WALLET ANALYSIS"}
          </button>
          {aiAnalysis&&(
            <div style={{marginTop:9,padding:"9px",borderRadius:8,animation:"fadein 0.3s ease",
              background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",
              fontSize:9,color:"rgba(255,255,255,0.5)",lineHeight:1.65}}>{aiAnalysis}</div>
          )}
        </div>
      ):(
        <div style={{textAlign:"center",padding:"20px 0",color:"rgba(255,255,255,0.12)",fontSize:9,lineHeight:1.9}}>
          <div style={{fontSize:20,marginBottom:8,opacity:0.2}}>◎</div>
          Click any node in the graph<br/>to inspect wallet activity
        </div>
      )}
    </div>
  );

  // Alerts panel — shared
  const AlertsPanel = ({flex=false}) => (
    <div style={{...card,display:"flex",flexDirection:"column",overflow:"hidden",
      ...(flex?{flex:1}:{minHeight:220})}}>
      <div style={{padding:"13px 15px 11px",borderBottom:"1px solid #141414",flexShrink:0,
        display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:12,fontWeight:700,color:"#fff"}}>Whale Alerts</div>
          <div style={{fontSize:9,color:"rgba(255,255,255,0.2)",marginTop:1}}>48h rolling window</div>
        </div>
        {alerts.length>0&&(
          <div style={{fontSize:11,color:"#F5C300",fontWeight:"bold",
            background:"rgba(245,195,0,0.09)",border:"1px solid rgba(245,195,0,0.2)",
            padding:"2px 9px",borderRadius:20,animation:"pulse 2.5s infinite"}}>{alerts.length}</div>
        )}
      </div>
      <div style={{overflowY:"auto",padding:"9px 11px 12px"}}>
        {alerts.length===0
          ?<div style={{textAlign:"center",padding:"16px 0",fontSize:9,color:"rgba(255,255,255,0.12)"}}>No alerts in window</div>
          :alerts.map((a,i)=>(
            <div key={i} onClick={()=>{
              const node=graph.nodes.find(n=>n.id===a.addr);
              if(node){setSelected(node);setAiAnalysis(null);}
            }} style={{padding:"9px 10px",marginBottom:6,borderRadius:9,cursor:"pointer",
              background:"rgba(245,195,0,0.03)",border:"1px solid rgba(245,195,0,0.09)",
              transition:"all 0.15s",animation:`fadein ${0.1+i*0.04}s ease`}}
              onMouseEnter={e=>e.currentTarget.style.background="rgba(245,195,0,0.08)"}
              onMouseLeave={e=>e.currentTarget.style.background="rgba(245,195,0,0.03)"}>
              <div style={{fontSize:8,color:"rgba(245,195,0,0.4)",marginBottom:3}}>{short(a.addr)}</div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <span style={{fontSize:12,color:"#F5C300",fontWeight:"bold"}}>{a.count} tokens moved</span>
                <span style={{fontSize:8,color:"rgba(245,195,0,0.35)",
                  background:"rgba(245,195,0,0.07)",padding:"2px 6px",borderRadius:4}}>≤48h</span>
              </div>
            </div>
          ))
        }
      </div>
    </div>
  );

  return (
    <div style={{display:"flex",flexDirection:"column",minHeight:"100vh",background:"#000",color:"#fff",
      fontFamily:"'DM Mono',monospace"}}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Mono:wght@300;400;500&family=Syne:wght@700;800&display=swap');
        *{box-sizing:border-box;margin:0;padding:0;}
        ::-webkit-scrollbar{width:3px;} ::-webkit-scrollbar-track{background:transparent;} ::-webkit-scrollbar-thumb{background:#222;border-radius:3px;}
        @keyframes pulse{0%,100%{opacity:.45;transform:scale(1)}50%{opacity:1;transform:scale(1.08)}}
        @keyframes fadein{from{opacity:0;transform:translateY(5px)}to{opacity:1;transform:none}}
      `}</style>

      {/* ── TOP BAR (always visible) ── */}
      <div style={{
        display:"flex",alignItems:"center",justifyContent:"space-between",
        padding: isMobile?"12px 14px":"14px 20px",
        borderBottom:"1px solid #141414",
        background:"#000",flexShrink:0,flexWrap:"wrap",gap:10,
      }}>
        {/* Logo */}
        <img src={LOGO_SRC} alt="Dope Ape Club"
          style={{height: isMobile?28:36, width:"auto", objectFit:"contain"}}/>

        <div style={{display:"flex",alignItems:"center",gap:8,marginLeft:"auto"}}>
          {demoMode
            ?<div style={{fontSize:9,padding:"4px 10px",borderRadius:6,letterSpacing:"0.07em",
                background:"rgba(245,195,0,0.07)",border:"1px solid rgba(245,195,0,0.2)",color:"#F5C300"}}>⚡ DEMO</div>
            :<div style={{fontSize:9,padding:"4px 10px",borderRadius:6,letterSpacing:"0.07em",
                background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.15)",color:"#fff",animation:"pulse 3s infinite"}}>● LIVE</div>
          }
          {/* Key button */}
          <div onClick={()=>setShowInput(v=>!v)} title="Connect Etherscan API key" style={{
            width:34,height:34,display:"flex",alignItems:"center",justifyContent:"center",
            borderRadius:9,cursor:"pointer",
            background:demoMode?"rgba(245,195,0,0.07)":"rgba(255,255,255,0.07)",
            border:`1px solid ${demoMode?"rgba(245,195,0,0.25)":"rgba(255,255,255,0.2)"}`,
          }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
              stroke={demoMode?"#F5C300":"#fff"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/>
            </svg>
          </div>
        </div>

        {/* Sub-line */}
        <div style={{width:"100%",fontSize:9,color:"rgba(255,255,255,0.18)",letterSpacing:"0.06em",marginTop:-4}}>
          {CONTRACT.slice(0,10)}…{CONTRACT.slice(-8)} · ERC-721 · Wallet Intelligence
        </div>
      </div>

      {/* ── API PANEL ── */}
      {showInput&&(
        <div style={{padding:"12px 16px",borderBottom:"1px solid #141414",background:"#000",flexShrink:0}}>
          <div style={{fontSize:9,color:"rgba(255,255,255,0.25)",marginBottom:8,letterSpacing:"0.08em"}}>
            CONNECT ETHERSCAN — PASTE API KEY FOR FULL HISTORY
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <input value={apiKey} onChange={e=>setApiKey(e.target.value)} onKeyDown={e=>e.key==="Enter"&&fetchLive()}
              placeholder="Your Etherscan API key…"
              style={{flex:1,minWidth:180,background:"rgba(255,255,255,0.04)",border:"1px solid #1C1C1C",
                color:"#fff",padding:"8px 12px",borderRadius:8,fontSize:10,fontFamily:"inherit",outline:"none"}}/>
            <button onClick={fetchLive} disabled={loading||!apiKey.trim()} style={{
              background:"rgba(245,195,0,0.1)",border:"1px solid rgba(245,195,0,0.35)",
              color:"#F5C300",padding:"8px 16px",borderRadius:8,fontSize:10,
              fontFamily:"inherit",cursor:loading?"wait":"pointer",letterSpacing:"0.05em",
              opacity:!apiKey.trim()?0.35:1}}>{loading?"LOADING…":"FETCH LIVE"}</button>
          </div>
          {error&&<div style={{marginTop:7,fontSize:9,color:"#f87171"}}>{error}</div>}
        </div>
      )}

      {/* ── BODY ── */}
      {isMobile ? (
        /* ── MOBILE LAYOUT — vertical stack ── */
        <div style={{flex:1,display:"flex",flexDirection:"column",padding:"12px",gap:10,overflowY:"auto"}}>

          {/* Stats 2x2 */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <Tile label="TOTAL TRANSFERS" value={stats.txs}     accent="#F5C300"/>
            <Tile label="UNIQUE WALLETS"  value={stats.wallets} accent="#FFF"/>
            <Tile label="TOKENS TRACKED"  value={stats.tokens}  accent="#888"/>
            <Tile label="MARKET SALES"    value={stats.mkt}     accent="#F5C300"/>
          </div>

          {/* Graph card */}
          <div style={{...card,display:"flex",flexDirection:"column"}}>
            <div style={{padding:"12px 14px 10px",borderBottom:"1px solid #141414",flexShrink:0}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:13,fontWeight:700,color:"#fff",marginBottom:6}}>
                Ownership Cluster
              </div>
              <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                {fBtn("all","All")}
                {fBtn("buysell","Mkt")}
                {fBtn("walletonly","Wallet→Wallet")}
              </div>
            </div>
            <div style={{position:"relative",height:300}}>
              <svg ref={svgRef} style={{width:"100%",height:"100%",display:"block"}}/>
              <div style={{position:"absolute",bottom:8,left:12,display:"flex",gap:10,
                fontSize:8,color:"rgba(255,255,255,0.2)"}}>
                {[["#fff","Wallet"],["#888","Mkt"],["#F5C300","⚠ Whale"]].map(([c,l])=>(
                  <span key={l}><span style={{color:c}}>●</span> {l}</span>
                ))}
              </div>
            </div>
          </div>

          {/* Wallet inspector below graph on mobile */}
          <InspectorPanel/>

          {/* Whale alerts */}
          <AlertsPanel/>

        </div>
      ) : (
        /* ── DESKTOP LAYOUT — sidebar ── */
        <div style={{flex:1,display:"flex",overflow:"hidden"}}>

          {/* Left nav */}
          <div style={{width:58,display:"flex",flexDirection:"column",alignItems:"center",
            padding:"16px 0",gap:6,background:"#000",borderRight:"1px solid #141414",flexShrink:0}}>
            {[
              "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6",
              "M13 7h8m0 0v8m0-8l-8 8-4-4-6 6",
              "M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9",
            ].map((path,i)=>(
              <div key={i} style={{
                width:38,height:38,display:"flex",alignItems:"center",justifyContent:"center",
                borderRadius:9,cursor:"pointer",
                background: i===0?"rgba(245,195,0,0.1)":"transparent",
                border:`1px solid ${i===0?"rgba(245,195,0,0.3)":"transparent"}`,
              }}>
                <svg width="17" height="17" viewBox="0 0 24 24" fill="none"
                  stroke={i===0?"#F5C300":"rgba(255,255,255,0.22)"} strokeWidth="1.5"
                  strokeLinecap="round" strokeLinejoin="round"><path d={path}/></svg>
              </div>
            ))}
          </div>

          {/* Main content */}
          <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",padding:"16px 18px 16px 14px",gap:11}}>

            {/* Stat tiles row */}
            <div style={{display:"flex",gap:10,flexShrink:0}}>
              <Tile label="TOTAL TRANSFERS" value={stats.txs}     accent="#F5C300"/>
              <Tile label="UNIQUE WALLETS"  value={stats.wallets} accent="#FFFFFF"/>
              <Tile label="TOKENS TRACKED"  value={stats.tokens}  accent="#888888"/>
              <Tile label="MARKET SALES"    value={stats.mkt}     accent="#F5C300"/>
            </div>

            {/* Body row */}
            <div style={{flex:1,display:"flex",gap:11,overflow:"hidden",minHeight:0}}>

              {/* Graph card */}
              <div style={{...card,flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
                  padding:"12px 16px 10px",borderBottom:"1px solid #141414",flexShrink:0,flexWrap:"wrap",gap:8}}>
                  <div>
                    <div style={{fontFamily:"'Syne',sans-serif",fontSize:13,fontWeight:700,color:"#fff"}}>Ownership Cluster</div>
                    <div style={{fontSize:9,color:"rgba(255,255,255,0.2)",marginTop:2}}>
                      {graph.nodes.length} wallets · {graph.links.length} connections
                    </div>
                  </div>
                  <div style={{display:"flex",gap:6}}>{fBtn("all","All")}{fBtn("buysell","Marketplace")}{fBtn("walletonly","Wallet → Wallet")}</div>
                </div>
                <div style={{flex:1,position:"relative",minHeight:0}}>
                  <svg ref={svgRef} style={{width:"100%",height:"100%",display:"block"}}/>
                  <div style={{position:"absolute",bottom:10,left:14,display:"flex",gap:14,
                    fontSize:9,color:"rgba(255,255,255,0.2)"}}>
                    {[["#fff","Wallet"],["#888","Marketplace"],["#F5C300","⚠ Whale"]].map(([c,l])=>(
                      <span key={l}><span style={{color:c}}>●</span> {l}</span>
                    ))}
                  </div>
                </div>
              </div>

              {/* Right column */}
              <div style={{width:256,display:"flex",flexDirection:"column",gap:10,flexShrink:0}}>
                <InspectorPanel/>
                <AlertsPanel flex/>
              </div>

            </div>
          </div>
        </div>
      )}
    </div>
  );
}
